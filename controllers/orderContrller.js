...
exports.getOrders = async (req, res) => {
  try {
    // Admin override: an admin may request another donor's orders by passing
    // ?userId=<id>. This lets the Admin dashboard feed the *same* statement
    // engine the donor uses, so both produce an identical statement.
    const isAdminRequest = req.user?.role === "admin" && !!req.query.userId;
    let targetUserId = req.user._id;
    let userEmail = req.user.email;
    if (isAdminRequest) {
      const targetUser = await User.findById(req.query.userId).select("email");
      if (!targetUser) {
        return res.status(404).json({
          status: "Error",
    ...
    const orders = await Order.find({
      $or: [
        { user: targetUserId },  // Orders linked to this user account
